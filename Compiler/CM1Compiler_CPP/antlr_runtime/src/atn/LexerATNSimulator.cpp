	...
	for (auto c : closure_->configs) {
		...
		for (size_t ti = 0; ti < n; ti++) { // for each transition
			...
			if (target != nullptr) {
				...
				Ref<LexerATNConfig> config = std::make_shared<LexerATNConfig>(static_cast<LexerATNConfig&>(*c), target, lexerActionExecutor);
...
atn::ATNState* LexerATNSimulator::getReachableTarget(not_null<Transition*> trans, size_t t) {
