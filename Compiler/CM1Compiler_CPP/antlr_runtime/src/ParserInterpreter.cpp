...
ParserRuleContext* ParserInterpreter::parse(size_t startRuleIndex)
{
	...
	if (startRuleStartState->isLeftRecursiveRule)
		enterRecursionRule(_rootContext.get(), startRuleStartState->stateNumber, startRuleIndex, 0);
	else
		enterRule(_rootContext.get(), startRuleStartState->stateNumber, startRuleIndex);
...
std::unique_ptr<InterpreterRuleContext> ParserInterpreter::createInterpreterRuleContext(ParserRuleContext* parent,
	size_t invokingStateNumber, size_t ruleIndex) {
	return std::make_unique<InterpreterRuleContext>(parent, invokingStateNumber, ruleIndex);
}
