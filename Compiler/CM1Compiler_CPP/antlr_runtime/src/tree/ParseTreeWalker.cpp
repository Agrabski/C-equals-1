...
void ParseTreeWalker::walk(not_null<ParseTreeListener*> listener, not_null<ParseTree*> t) const
{
	if (is<ErrorNode*>(t))
	{
		listener->visitErrorNode(dynamic_cast<ErrorNode*>(t.get()));
		return;
	}
	else
		if (is<TerminalNode*>(t))
		{
			listener->visitTerminal(dynamic_cast<TerminalNode*>(t.get()));
			return;
		}
