...
namespace antlr4 {
	namespace tree {
		...
		public:
			...
			/// node payloads to get the text for the nodes.  Detect
			/// parse trees and extract data appropriately.
			static std::string toStringTree(ParseTree* t, const std::vector<std::string_view>& ruleNames);
			static std::string getNodeText(ParseTree* t, Parser* recog);
			static std::string getNodeText(ParseTree* t, const std::vector<std::string_view>& ruleNames);
